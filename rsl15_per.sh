...
slaveSerial=${BRD2_HCI}
masterSerial=${BRD1_HCi}

echo slaveSerial
echo masterSerial

python3 $MSDK/Tools/Bluetooth/dtm_sweep.py ${slaveSerial} ${masterSerial} ${resultFile}

python3 $MSDK/Tools/Bluetooth/BLE_hci.py ${slaveSerial} -c "reset; exit"
python3 $MSDK/Tools/Bluetooth/BLE_hci.py ${masterSerial} -c "reset; exit"


#unlock the resources if we got the in the first place

echo "Try to unlock the hardware resources."
...
#plot results
python3 ~/Workspace/yc/msdk_ci_per_tool/plot_per_results.py ${resultFile} "OnSemi, RSL15 DTM PER Data" "rls15_per_data"